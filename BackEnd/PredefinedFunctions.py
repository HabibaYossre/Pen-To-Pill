...
def submit(images):
    ...
    corrected_texts = correct_text_with_bart(extracted_texts, tokenizer=bart_tokenizer, model=bart_model, device=device)
    for corrected in corrected_texts:
        results.append(split_into_medicine_dosage_pairs(corrected))
